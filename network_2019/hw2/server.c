...
char playername[100][16]={"stdin","stdout","stderr","socket", "player1","player2","player3","player4","player5","player6"};
void *gamemenu();
int client_number;
short online[100];
short playing[100];
...
void *gamemenu(void *arg)
{
	...
	while(1){
		...
		if(strcmp(buffer, "quit")==0){
			...
			for(int i=0; i<10;i++){
				if(online[i]){
					strcat(user, playername[i]);
					if(i==clientSocket) strcat(user, "<--you");
					if(playing[i]>0){
						strcat(user, "\t(Playing with ");
						strcat(user, playername[playing[i]]);
						strcat(user, ")");
					}
					strcat(user, "\n");
				}
			}
			write(clientSocket, user, sizeof(user));
		}
			...
			if(player_sock<0){
				...
				if(playing[player_sock]<0){
					...
					strcat(data, " invite you to join the game (Y/N) ? ");
