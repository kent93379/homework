...
void *sendsock(void *arg)
{
	...
	while(1){
		...
		if(strcmp(command, "quit")==0){
			...
			printf("Choose a player you want to invite : ");
			scanf("%s", playername);
			strcat(command, " ");
...
void playing(int socket)
{
	...
	while(1){
		...
		if(strcmp(data, "quit")==0 || strcmp(data, "Win")==0 || strcmp(data, "Lose")==0 || strncmp(data, "Even", 4)==0 || strncmp(data, "Leave", 5)==0){
			...
			while(1){
				printf("You turn !! (you can print -1 to leave the game) ");
				scanf("%d", &next);
				if(next>=1 && next<=9 || next==-1) break;
				else printf("<<< Error move !! Please enter the correct number(from  1 to 9) !! >>>\n\n");
			}
